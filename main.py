...
async def pm_menu_keyboard():
    ...
    p79 = prices.get('79', 0)
    p99 = prices.get('99', 0)
    p299 = prices.get('299', 0)

    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"🎁 42 lik — {p42} so'm ({c42} ta bor)", callback_data="buy_42")],
        [InlineKeyboardButton(text=f"🎁 79 lik — {p79} so'm ({c79} ta bor)", callback_data="buy_79")],
        [InlineKeyboardButton(text=f"🎁 99 lik — {p99} so'm ({c99} ta bor)", callback_data="buy_99")],
        [InlineKeyboardButton(text=f"🎁 299 lik — {p299} so'm ({c299} ta bor)", callback_data="buy_299")]
    ])
...
# --- BALANS VA CHEK ORQALI TO'LOV ---

@dp.message(F.text == "💳 Balans")
async def show_balance(message: types.Message):
    balance = await db.get_user_balance(message.from_user.id)
    await message.answer(
        f"🆔 Sening Telegram ID'ingiz: `{message.from_user.id}`\n"
...
async def process_amount(message: types.Message, state: FSMContext):
    ...
    if amount < 1000 or amount > 1000000:
        await message.answer(
            "❌ Minimal 1 000 so'm, maksimal 1 000 000 so'm kiritishingiz mumkin!",
...
async def admin_balance_add_amount(message: types.Message, state: FSMContext):
    ...
    await message.answer("✅ Balans qo'shildi!", reply_markup=admin_menu_keyboard())
    await state.clear()
...
async def admin_balance_sub_amount(message: types.Message, state: FSMContext):
    ...
    amount = int(message.text)
    await db.add_user_balance(target_id, -amount)
    await message.answer("✅ Balans ayirildi!", reply_markup=admin_menu_keyboard())
    await state.clear()
...
async def admin_add_pm_category(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"📥 {cat}-lik uchun PM kodlarini har birini yangi qatordan yozib yuboring:",
        reply_markup=back_keyboard()
    )
